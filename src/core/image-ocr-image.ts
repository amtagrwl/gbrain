...
export const IMAGE_OCR_PROVIDER_MAX_DIMENSION = 8_000;
export const IMAGE_OCR_LOCAL_MAX_PIXELS = 25_000_000;
...
export type BoundedImageOcrFormat = 'png' | 'jpeg' | 'gif' | 'webp' | 'heic' | 'avif';

export interface BoundedImageOcrInfo {
...
export function validateAndPriceImageOcrDimensions(
  ...
  // Anthropic's standard-resolution rule first scales to both a 1,568px
  // longest edge and about 1.15MP, then prices visual input at roughly one
  // token per 750 scaled pixels. Ceil dimensions and token count so local
  // admission is conservative around every rounding boundary.
...
function checkedInfo(
  ...
  return { format, frameCount: 1, ...validateAndPriceImageOcrDimensions(width, height) };
}
...
function inspectPng(bytes: Buffer): BoundedImageOcrInfo {
  ...
  while (offset < bytes.length) {
    ...
    if (type === 'IHDR') {
      ...
      validateAndPriceImageOcrDimensions(width, height);
    } else if (type === 'IDAT') {
...
function inspectGif(bytes: Buffer): BoundedImageOcrInfo {
  ...
  validateAndPriceImageOcrDimensions(canvasWidth, canvasHeight);
  ...
  while (offset < bytes.length) {
    ...
    if (introducer === 0x2c) {
      ...
      validateAndPriceImageOcrDimensions(frameWidth, frameHeight);
...
function inspectWebp(bytes: Buffer): BoundedImageOcrInfo {
  ...
  while (offset < bytes.length) {
    ...
    if (type === 'VP8X') {
      ...
      validateAndPriceImageOcrDimensions(canvasDimensions.width, canvasDimensions.height);
    } else if (type === 'VP8 ') {
      if (
        length < 10
